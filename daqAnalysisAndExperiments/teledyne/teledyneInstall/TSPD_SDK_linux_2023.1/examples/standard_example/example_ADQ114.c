...
void adq114_multi_record(void *adq_cu, int adq_num);
void adq114_streaming(void *adq_cu, int adq_num);
//void adq114_time_stamp(void *adq_cu, int adq_num);
...
void adq114(void *adq_cu, int adq_num)
{
  int mode;
  int* revision = ADQ_GetRevision(adq_cu, adq_num);
  printf("\nConnected to ADQ114 #1\n\n");
  ...
  {
  case 1:
    adq114_multi_record(adq_cu, adq_num);
    break;
  case 2:
    adq114_streaming(adq_cu, adq_num);
    break;
  default:
    return;
    break;
...
error:
  ...
  if (exit == 1)
  {
    ...
    int page_count = ADQ_GetPageCount(adq_cu, adq_num);
...
void adq114_streaming(void *adq_cu, int adq_num)
{
  ...
  CHECKADQ(ADQ_SetDataFormat(adq_cu, adq_num,ADQ114_DATA_FORMAT_UNPACKED_14BIT));
  ...
  CHECKADQ(ADQ_SetStreamStatus(adq_cu, adq_num,ADQ114_STREAM_ENABLED));
  CHECKADQ(ADQ_ArmTrigger(adq_cu, adq_num));
  ...
  // ADC channel data is interleaved in 8 bytes chunks (4 samples with unpacked 14-bit or 16-bit modes)
  // These samples must be interleaved before writing to file.
  ...
  while (samples_to_collect>0)
  {
    ...
    for (i=0; (i<4) && (samples_to_collect>0); i++)
    {
      ...
      samples_to_collect--;
      if (samples_to_collect >= 4)
      {
        fprintf(outfile, "%d\n", (int)data_stream_target[n_samples_collect-samples_to_collect+4]);
      }
      else
      {
    ...
    samples_to_collect -= MIN(4,samples_to_collect); //Decrease by 4 but don't pass zero.
  }
...
//.................................
  ...
  FILE* outfile, stampdelta;
  outfile = fopen("data.out", "w");
  //stampdelta = fopen("deltadata.out", "w"); //to store delta time stamp data
...
//...............................
  ...
  if (trig_mode == 1) //SW Trigger Mode
  {
    int trig;
    printf("Automatically triggering your device to collect data.\n");
    CHECKADQ(ADQ112_DisarmTrigger(adq_cu, adq_num));
    CHECKADQ(ADQ112_ArmTrigger(adq_cu, adq_num));
    do
    {
      CHECKADQ(ADQ112_SWTrig(adq_cu, adq_num)); //Trig multiple times until all records are triggered
      trig = ADQ112_GetAcquiredAll(adq_cu, adq_num);
      switch (trig_time_mode)
      {
      case 0 :
        {
          CHECKADQ(ADQ112_ReadGPIO(adq_cu, adq_num));
          synctrig = ADQ112_ReadGPIO(adq_cu, adq_num);

          CHECKADQ(ADQ112_SetTrigTimeMode(adq_cu, adq_num,0));//continious count
          CHECKADQ(ADQ112_DisarmTrigger(adq_cu, adq_num));
          CHECKADQ(ADQ112_ArmTrigger(adq_cu, adq_num));
          CHECKADQ(ADQ112_GetTrigTime(adq_cu, adq_num));

          do{
            CHECKADQ(ADQ112_GetTrigTime(adq_cu, adq_num));
            od_timest.push_back(ADQ112_GetTrigTime(adq_cu, adq_num));
          }while (!synctrig);

          do{
            if (restart_mode == 1)
            {
            CHECKADQ(ADQ112_ResetTrigTimer(adq_cu, adq_num,1));
            }
            else if (restart_mode == 0)
            {
            CHECKADQ(ADQ112_ResetTrigTimer(adq_cu, adq_num,0));
            }
          }while (synctrig);
      ...
      case 1 :
        {
          ...
          CHECKADQ(ADQ112_SetTrigTimeMode(adq_cu, adq_num,1));//activate sync mode
          CHECKADQ(ADQ112_DisarmTrigger(adq_cu, adq_num));
          CHECKADQ(ADQ112_ArmTrigger(adq_cu, adq_num));
          ...
          do{
            ...
            CHECKADQ(ADQ112_ArmTrigger(adq_cu, adq_num));
            CHECKADQ(ADQ112_GetTrigTime(adq_cu, adq_num));
            //CHECKADQ(ADQ112_GetTrigTimeCycles(adq_cu, adq_num));
            CHECKADQ(ADQ112_GetTrigTimeSyncs(adq_cu, adq_num));
  ...
  else // level & external
  {
    ...
    do
    {
      trigged = ADQ112_GetAcquiredAll(adq_cu, adq_num);
      switch (trig_time_mode)
      {
      case 0 :
        { //sync off
