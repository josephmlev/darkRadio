...
void adq108_multirecord(void *adq_cu, int adq_num);
void adq108_microtca_functionality_demo(void *adq_cu, int adq_num);
void adq108_streaming(void *adq_cu, int adq_num);
...
void adq108(void *adq_cu, int adq_num)
{
  int mode = 0;
  int* revision = ADQ_GetRevision(adq_cu, adq_num);
  printf("\nConnected to ADQ108 #1\n\n");
  ...
  {
  case 1:
    adq108_multirecord(adq_cu, adq_num);
    break;
  case 2:
    adq108_streaming(adq_cu, adq_num);
    break;
  case 3:
    adq108_microtca_functionality_demo(adq_cu, adq_num);
    break;
  default:
    return;
    break;
...
void adq108_streaming(void *adq_cu, int adq_num)
{
  ...
  CHECKADQ(data_stream_target = (signed char*)malloc(n_samples_collect*sizeof(signed char)));
  ...
  while (samples_to_collect > 0)
  {
    do
    {
      collect_result = ADQ_GetTransferBufferStatus(adq_cu, adq_num, &buffers_filled); // Does not work for USB3 yet
      if (collect_result)
      {
        printf("Filled: %2u\n", buffers_filled);
        retry += 1;
  ...
  for (LoopVar = 0; LoopVar < samples_to_collect; LoopVar += 1)
  {
    ...
    for (ik = 0; ik < 1; ik++)
    {
      ...
      fprintf(outfile, "%d", data_stream_target[LoopVar + ik]);
