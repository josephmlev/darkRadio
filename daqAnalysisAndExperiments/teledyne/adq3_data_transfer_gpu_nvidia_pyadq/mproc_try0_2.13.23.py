...
def main():
    ...
    print(f"memmory handles: {memory_handles}")
    print()
    ...
    transfer_thread = threading.Thread(target = transfer_data, args=(dev, parameters, sem,))
    check_buffer_thread = threading.Thread(target=check_buffer, args = (dev, sem,))
    check_buffer_thread.start()
    transfer_thread.start()
    #transfer_thread(dev, parameters)
...
def check_buffer(dev, sem):
    for i in range(10):
        #sem.acquire()
        status = pyadq.ADQP2pStatus()._to_ct()
        print('status', dev.ADQ_WaitForP2pBuffers(ct.byref(status), 0))
        time.sleep(.000)
...
def transfer_data(dev, parameters, sem):
    ...
    gbps = bytes_received / total_time
    dev.ADQ_StopDataAcquisition()
