...
class FpgaClient(CallbackClient):
    ...
    def tap_start(self, tap_dev, device, mac, ip, port):
        ...
        if reply.arguments[0].decode()=='ok': return
        else: raise RuntimeError("Failure starting tap device %s with mac %s, %s:%s"%(device,mac_str,ip_str,port_str) + str(reply.arguments[0]) + str(len(reply.arguments[0])))
    ...
    def upload_bof(self, bof_file, port, force_upload = False, timeout = 30):
        ...
        def makerequest(result_queue):
            try:
                ...
                if(result[0].arguments[0] == Message.OK):
                    result_queue.put('', block = False)
                else:
                    result_queue.put('Request to client returned, but not Message.OK.', block = False)
            except:
                result_queue.put('Request to client failed.', block = False)
        def uploadbof(filename, result_queue):
            ...
            while (not connected) and (time.time() - stime < 2):
                try:
                    ...
                    time.sleep(0.1)
            if not connected:
                result_queue.put('Could not connect to upload port.', block = False)
            try:
                upload_socket.send(open(filename).read())
            except:
                result_queue.put('Could not send file to upload port.', block = False)
            result_queue.put('', block = False)
        # request thread
        ...
        request_result = request_queue.get(block = False)
        upload_result = upload_queue.get(block = False)
    ...
    def blindwrite(self, device_name, data, offset=0):
        """Unchecked data write.
           ...
           """
        #data = str(data)   
        #assert (type(data)==str) , 'You need to supply binary packed string data! IT IS ' + str(data)
        assert (len(data)%4) ==0 , 'You must write 32bit-bounded words!' + data
    ...
    def read_byte(self, device_name, offset=0):
        """Calls .read() command with size=4, offset=0 and
           unpacks returned four bytes into signed 32bit integer.
           ...
           @return  Integer: value read.
           """
        data = self.read(device_name, 1, offset*4)
