...
class fbf:
	...
	def get_bfs(self):
		...
		all_bfs = list(range(self.get_param('bf_be_per_fpga')))
		return all_bfs
	...
	def antenna2antenna_indices(self, beam, ant_strs=all):
		...
		if ant_strs==all:
			antenna_indices.extend(list(range(n_ants)))
...
#        print 'bf_write_int: disabling everything' 
		...
		#expand, check and convert to input indices 
			...
			for antenna_index in antenna_indices:
				...
				if all_freqs:
					...
					bins = list(range(0, n_bfs*bf_fft_bins, bf_fft_bins))
	...
	def cf_bw2fft_bins(self, centre_frequency, bandwidth):
		#returns fft bins associated with provided centre_frequency and bandwidth centre_frequency is assumed to be the centre of the 2^(N-1) fft bin"""
		bins = []
		...
		bins = list(range(edge_bins[0], edge_bins[1]+1))
...
#   CALIBRATION 
	...
	def cal_default_get(self, beam, ant_str):
		...
		if cal_default == 'coeffs':
			...
			calibration = numpy.polyval(poly, list(range(n_coeffs)))
	...
	def cal_spectrum_set(self, beam, ant_str, init_coeffs = [], init_poly = [], spead_issue=True):
		...
		if init_coeffs == [] and init_poly == []: coeffs = self.cal_default_get(beam=beam, ant_str=ant_str)
			...
			coeffs = numpy.polyval(init_poly, list(range(n_coeffs)))
