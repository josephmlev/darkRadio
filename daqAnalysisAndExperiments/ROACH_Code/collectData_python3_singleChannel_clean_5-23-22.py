...
"""talk with sign gen code. Not needed for normal use, good for testing
...
#open 10Gbe line, set up UDP connection with roach
...
def gbe0(memName, writeName, startName, endName):
    """DAQ from ROACH. Using shared memory buffer to pass data to GPU.
    Check flag bit to see when to write. Reset flag after writing

    Args:
    memName -- Shared memory object. Inherited from multiprocess
    writeName -- Shared memory object. Inherited from multiprocess.
        Boolian array.
    """
    startTime = time.time()
    toggle = False
    adc_mem = shared_memory.SharedMemory(name=memName.name)
    sharedData =  np.ndarray(
        (
        ROACH_SETUP.NUM_BUFFERS, 
        ROACH_SETUP.NUM_FFT, 
        int((ROACH_SETUP.PAYLOAD_LEN)*ROACH_SETUP.DATA_ITS*8*ROACH_SETUP.EXTRA_MULT),
        ), 
        dtype=np.dtype('i1'), 
        buffer=adc_mem.buf) 
    ...
    while not(endArr[0]) and (time.time() - startTime < ROACH_SETUP.TOTAL_TIME):
        ...
        try:
            ...
            """ Print/plot stats about time to get data
            testTime = time.time()
...
def createNewFile(fileName):
    """Creates a new H5 file and sets its atributes
    Args:
    fileName -- string
        Name of file
    ...
    To do. add hard code values to config file
    Some of these values are reset by config file in writeDataNew
    """
    ...
    runInfo.attrs['averages'] = str(ROACH_SETUP.NUM_FFT)
    acquisitionLength = (1000*ROACH_SETUP.DATA_ITS * ROACH_SETUP.PAYLOAD_LEN*8
        / ROACH_SETUP.CLOCK_SPEED)
    runInfo.attrs['acquisition length'] = str(round(acquisitionLength, 3)) + 'ms' 
...
def takeRigolData(rigolQueue, endName):
    ...
    Args: 
    rigolQueue -- multiprocess queue
        shared memory object to put rigol memory into. Similar to memName.
    endName -- Shared memory object. Bool
        Flag to stop writing.

    To do: maker sure all of the things we care about are written
    dont hard code ip address"""
...
def plotCurrentAcq(dataMem, endName, plotQueue):
    """
    Args:
    dataMem -- shared memory object. 
        Contains NUM_BEFORE_TOGGLE RMS averaged magnitude spectra 
    endName -- shared memory object. Bool
        Flag to stop writing.
    plotQueue -- multiprocess queue
        creates and desplays data in event desplay 

    to do: make a consitant window size and zoom level
    dont hardcode save locations.
    """
    ...
    #generate layout. Add more comments 
    ...
    freqs = (np.asarray(range(2**(ROACH_SETUP.FFT_POW-1)))
        *ROACH_SETUP.CLOCK_SPEED/10**6/(2**ROACH_SETUP.FFT_POW))

    #average to only plot 1/8th of the total points for speed.
    numAvg = 2**3
    removeBins = 1 
...
def writeDataNew(dataMem, rigolQueue, endName, plotQueue):
    """Saves data frame to file
    Args:
    dataMem -- shared memory object. Spectrum
        Contains NUM_BEFORE_TOGGLE RMS averaged magnitude spectra
    rigolQueue -- shared memory object.
        Queue that rigol data is stored in
    endName -- shared memory object. Bool
        Flag to stop writing.
    plotQueue -- multiprocess queue
        creates and desplays data in event desplay 
