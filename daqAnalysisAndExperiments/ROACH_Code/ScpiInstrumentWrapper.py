...
class PrologixInstrument(object):
	...
	def __init__(self, gpibAddr, comPort, baud_rate=921600, timeout=0.25, silent=True):
		...
		PrologixInstrument.serFd.write(("++mode 1"+self.lineEnd).encode())
		PrologixInstrument.serFd.write(("++ifc"+PrologixInstrument.lineEnd).encode())
		#serialInterface.serFd.write("++auto 1"+serialInterface.lineEnd )
		PrologixInstrument.serFd.write(("++read_tmo_ms 200"+PrologixInstrument.lineEnd).encode())
		#ver = tuple(( int(i) for i in self.query('++ver').split()[-1].split('.').encode()))
		#if ver != (6, 107):
	...
	def query(self,cmd):
		PrologixInstrument.serFd.write(('++addr' + str(self.gpibAddr) + PrologixInstrument.lineEnd).encode() )
		if isinstance(cmd,list):
			...
			if not self._silent:
				print("Command: \'%s\'" % cmd)
			PrologixInstrument.serFd.write((cmd+PrologixInstrument.lineEnd).encode())
		#serialInterface.serFd.write("++read eoi"+serialInterface.lineEnd)
		PrologixInstrument.serFd.write(("++read"+PrologixInstrument.lineEnd).encode())
		retry=1
		while retry>0:
